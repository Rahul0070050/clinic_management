...
function Signup() {
    ...
    const [userDataErr, setUserDataErr] = useState({ username: false, password: false, confirmPassword: false, email: false });
    const [showPassword, setShowPassword] = useState(false);
    const [recheckConfirmPassword, setRecheckConfirmPassword] = useState(false);
    ...
    function handleSubmit(e) {
        ...
        if (userData.password != userData.confirmPassword) {
            setRecheckConfirmPassword(prev => !prev);
            return;
        }
        ...
        if (userData.username == "" || userData.password == "" || userData.confirmPassword == "" || userData.email == "") {
            setUserDataErr(prev => {
                return {
                    ...
                    confirmPassword: userData.confirmPassword == "" ? true : false
                }
            })
            return;
        }
        ...
        if (userData.password.length < 8) {
            setErr("password must contain minimum 8 letter");
            return;
        }
            ...
            <div className="user-signup__container">
                ...
                <div className="container__child signup__thumbnail">
                    <div className="thumbnail__logo">
                        ...
                        <h1 className="logo__text">skin care clinic</h1>
                    </div>
                    <div className="thumbnail__content text-center">
                        <h1 className="heading--primary">Welcome to SCC.</h1>
                        <h2 className="heading--secondary">We will take care for your skin</h2>
                    </div>
                ...
                <div className="container__child signup__form">
                    <form>
                        <div className="form-group">
                            <label htmlFor="username" style={{ color: userDataErr.username ? "red" : "black" }}>Username {userDataErr.username ? <span>username is required</span> : null}</label>
                            <input className="form-control" onChange={handleOnchange} type="text" name="username" id="username" placeholder="username" />
                        </div>
                        <div className="form-group">
                            <label htmlFor="email" style={{ color: userDataErr.email ? "red" : "black" }}>Email {userDataErr.email ? <span>email is required</span> : null}</label>
                            <input className="form-control" onChange={handleOnchange} type="text" name="email" id="email" placeholder="id@gmail.com" />
                        </div>
                        <div className="form-group">
                            <label htmlFor="password" style={{ color: userDataErr.password ? "red" : "black" }}>Password {userDataErr.password ? <span>password is required</span> : null}</label>
                            <input className="form-control" onChange={handleOnchange} type={showPassword ? "text" : "password"} name="password" id="password" placeholder="*********" />
                            <img className='password-visibility' onClick={() => setShowPassword(prev => !prev)} src={showPassword ? openEye : closedEYe} alt="" />

                        </div>
                        <div className="form-group">
                            <label htmlFor="passwordRepeat" style={{ color: recheckConfirmPassword ? (userData.password == userData.confirmPassword ? "black" : "red") : "black" }}>Repeat Password {recheckConfirmPassword ? (userData.password != userData.confirmPassword ? <span> * is not matching</span> : null) : null}</label>
                            <input className="form-control" onChange={handleOnchange} type={showPassword ? "text" : "password"} name="confirmPassword" id="passwordRepeat" placeholder="*********" />
                        </div>
                        <div className="m-t-lg">
                            <ul className="list-inline">
                                <li>
                                    <input className="btn btn--form" type="button" onClick={handleSubmit} value="Register" />
                                </li>
                                <li>
                                    <Link className="signup__link" href="/login">I am already an account</Link>
                                </li>
                            </ul>
                        </div>
                    </form>
